...
static unsigned int termistor_adc;
static int cooler_state=0;
...
CY_ISR(ADC_ISR_func) {
	...
	if(isr_flags & ADC_EOS_MASK) {
        if(conversion_counter < 100) {
    		...
    		termistor_adc=ADC_GetResult16(ADC_CHAN_TEMP)>> adc_mix_ratio;
            if(cooler_state)
            {
                if(termistor_adc<=settings.cooler_off_adc_data)
                {
                    cooler_state=0;
                    cooler_Write(0);
                }
            ...
            {
                if(termistor_adc>=settings.cooler_on_adc_data)
                {
                    cooler_state=1;
                    cooler_Write(1);
                }
...
uint16 get_termistor() {
	return termistor_adc;
}
int get_cooler_state() {
	return cooler_state;
}
